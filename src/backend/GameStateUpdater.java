...
public class GameStateUpdater {
	...
	public static GameMap nextState(GameMap currentGameMap, GameStep cGameStep) {
		...
		System.out.println("Current GameStep: "
				+ currentGameStep.getDate().getTime());
		// Changes for single User
		for (Participation p : nextGameMap.getParticipations()) {

			/* Creation */
			// p.setBases(createBasesAndBuildings(p.getBases())); there is
			// nothing to calculate
			// p.setTroops(createTroops(p.getTroops())); there is nothing to
			// calculate
			// because the fontend shows everything that has an lower gamestep
			// than the current gamestep
				...
				for (Troop t : troops) {
					if (b.getSquare().equals(t.getCurrentSquare())) {
						if (t.getParticipation().equals(p)) {
							...
							opponentStrength += t.getUpgradeLevel()
									.getStrength();
							attack.add(t);
						}
					}
				...
				if (opponentStrength > 0) {
					if (myStrength >= opponentStrength) {
						while (!attack.isEmpty()) {
							...
							attack.get(0).getCurrentSquare().getTroops()
									.remove(attack.get(0));
							op.getTroops().remove(attack.get(0));
							troops.remove(attack.get(0));
							if (attack.get(0).getCreated() != null) {
						...
						while (!defense.isEmpty()) {
							...
							defense.get(0).getCurrentSquare().getTroops()
									.remove(defense.get(0));
							troops.remove(defense.get(0));
							if (defense.get(0).getCreated() != null) {
								System.out.println("Kill Troop: "
										+ defense.get(0).getId());
							}
							...
							for (ResourceAmount r : p.getResources()) {
								...
								for (Participation o : plist) {
									for (ResourceAmount or : o.getResources()) {
										if (or.getResource().equals(
												r.getResource())) {
											or.setAmount(or.getAmount()
													+ r.getAmount());
										}
									}
								}
	...
	/**
	 * Ressources: +100 Standard + XX for every Building that produces that
	 * Resource + 50 for every privileged Square for that Resource
	 * 
	 * @param resources
	 * @param bases
	 * @return
	 */
	public static List<ResourceAmount> createRessources(
			List<ResourceAmount> resources, List<Base> bases) {
			...
			for (Base b : bases) {
				...
				if (b.getBuildings() != null) {
					for (Building bu : b.getBuildings()) {
						if (bu.getType().getProductionType()
								.equals(r.getResource())) {
							r.setAmount(r.getAmount()
									+ (bu.getType().getProductionRate() * bu
											.getUpgradeLevel()));
						}
					}
				}
				// Privileged Squares
				if (b.getSquare().getPrivilegedFor() != null
						&& b.getSquare().getPrivilegedFor()
								.equals(r.getResource())) {
	...
	/**
	 * Upgrade: if LevelUpgradeFinish == null then no upgrade if
	 * LevelUpgradeFinish <= gamestep then upgradelevel + 1
	 * 
	 * @param bases
	 * @return
	 */
	private static List<Base> upgradeBuildings(List<Base> bases) {
		...
		for (Base b : nextBases) {
			for (Building bu : b.getBuildings()) {
				if (bu.getLevelUpgradeFinish() != null)
					System.out.println("Building " + bu.getId() + " Upgrade: "
							+ bu.getLevelUpgradeFinish().getDate().getTime());
				if (bu.getLevelUpgradeFinish() != null
						&& bu.getLevelUpgradeFinish().getDate().getTime() <= currentGameStep
								.getDate().getTime()) {
	...
	/**
	 * Upgrade: if LevelUpgradeFinish == null then no upgrade if
	 * LevelUpgradeFinish <= gamestep then upgradelevel = upgradelevel.nextLevel
	 * 
	 * @param troops
	 * @return
	 */
	private static List<Troop> upgradeTroops(List<Troop> troops) {
		...
		for (Troop t : nextTroops) {
			if (t.getLevelUpgradeFinish() != null)
				System.out.println("Troop " + t.getId() + " Upgrade: "
						+ t.getLevelUpgradeFinish().getDate().getTime());
			if (t.getLevelUpgradeFinish() != null
					&& t.getLevelUpgradeFinish().getDate().getTime() <= currentGameStep
							.getDate().getTime()) {
	...
	/**
	 * Troops move to targetsquare if getMovementFinish != null and <=
	 * currentGameStep
	 * 
	 * @param troops
	 * @return
	...
	private static List<Troop> moveTroops(List<Troop> troops) {
		...
		for (Troop t : nextTroops) {
			if (t.getMovementFinish() != null)
				System.out.println("Troop" + t.getId() + " Movement: "
						+ t.getMovementFinish().getDate().getTime());
			if (t.getMovementFinish() != null
					&& t.getMovementFinish().getDate().getTime() <= currentGameStep
							.getDate().getTime()) {
				t.getCurrentSquare().getTroops().remove(t);
	...
	private static List<Base> destroyBases(List<Base> bases) {
		...
		for (int i = 0; i < nextBases.size(); i++) {
			if (nextBases.get(i).getDestroyed() != null
					&& nextBases.get(i).getDestroyed().getDate().getTime() <= currentGameStep
							.getDate().getTime()) {
				nextBases.get(i).getSquare().setBase(null);
				if(nextBases.get(i).getCreated()!=null){
					System.out.println("Destroy Base: " + nextBases.get(i).getId());
				}
	...
	public static Participation getWinner(List<Troop> fighters) {
		...
		for (Troop t : fighters) {
			if (strengths.containsKey(t.getParticipation())) {
				strengths.put(t.getParticipation(),
						strengths.get(t.getParticipation())
								+ t.getUpgradeLevel().getStrength());
			} else {
				strengths.put(t.getParticipation(), t.getUpgradeLevel()
						.getStrength());
			}
		}
