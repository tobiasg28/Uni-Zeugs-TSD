...
public class GameStart {
    ...
    // Precondition: Types for Troops, Ressources, Buildings already exist
    public static long newGame(String name, int maxPlayer) throws GameStartException {
            ...
            for (int y = 0; y < (2 * maxPlayer); y++) {
                ...
                try {
                    if (sDAO.create(s)) {
                        ...
                        throw new GameStartException("ERROR: create Square returns false");
                    }
                } catch (DAOException ex) {
    ...
    public static boolean newPlayer(User user, Long GameMapId) throws GameStartException {
        GameMapDAO mDao = new GameMapDAO();
        GameMap map = null;
        try {
        ...
        for (Participation p : map.getParticipations()) {
            if (user.equals(p.getParticipant())) {
                throw new GameStartException("ERROR: Participation already exists");
            }
        }
        ...
        for (Resource r : resources) {
            ...
            try {
                if (raDao.create(ra)) {
                    ...
                    throw new GameStartException("ERROR: create ResourceAmount returns false");
                }
            } catch (DAOException ex) {
        ...
        try {
            if (bDao.create(start)) {
                ...
                throw new GameStartException("ERROR: create StarterBase returns false");
            }
        } catch (DAOException ex) {
        ...
        try {
            if(tDao.create(t)){
                ...
                throw new GameStartException("ERROR: create Troop returns false");
            }
        } catch (DAOException ex) {
        ...
        try {
            if (pDao.create(player)) {
                ...
                throw new GameStartException("ERROR: create Participation returns false");
            }
        } catch (DAOException ex) {
            throw new GameStartException("ERROR: create Participation or update", ex);
        }
    }
}
